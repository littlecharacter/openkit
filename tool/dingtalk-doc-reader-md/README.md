...
使用 Node.js + Playwright 复用钉钉登录态，读取钉钉文档正文、评论和图片资源，并导出为 Markdown / JSON / 图片文件。
...
- 正文优先读取钉钉文档真实接口，再尽量保留标题、列表、表格、链接、行内样式和图片的 Markdown 结构
- 正文与评论中的图片会单独下载到 `assets/` 目录，并在 Markdown 中以相对路径引用
- 最终只输出一个 `document.md`，其中包含正文和评论两个部分
